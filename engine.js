...
export class BodyEngine{
 ...
 async loadMacroStack(){
  ...
  const meta=await import("./exact-macro-meta.js?v=4.0.2");
 ...
 async loadAdvancedData(){
  ...
  const [cfg,data]=await Promise.all([import("./modifier-config.js?v=4.0.2"),import("./body-morphs.js?v=4.0.2")]);
  this.groups=cfg.GROUPS;this.directData=data.DIRECT;
  this.onProgress("Detailparameter …","Face");
  const [fc,fd]=await Promise.all([import("./face-config.js?v=4.0.2"),import("./face-morphs.js?v=4.0.2")]);
  this.faceGroups=fc.FACE_GROUPS;this.faceData=fd.FACE;
 }
 async loadRig(){
  this.onProgress("Rig lädt …","MakeHuman default");
  const mod=await import("./rig-data.js?v=4.0.2");this.rig=mod.RIG;
