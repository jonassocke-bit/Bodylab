...
export class BodyEngine{
 ...
 async loadMacroStack(){
  ...
  const meta=await import("./exact-macro-meta.js?v=3.21.6");
 ...
 async loadAdvancedData(){
  ...
  const [cfg,data]=await Promise.all([import("./modifier-config.js?v=3.21.6"),import("./body-morphs.js?v=3.21.6")]);
  this.groups=cfg.GROUPS;this.directData=data.DIRECT;
  this.onProgress("Detailparameter …","Face");
  const [fc,fd]=await Promise.all([import("./face-config.js?v=3.21.6"),import("./face-morphs.js?v=3.21.6")]);
  this.faceGroups=fc.FACE_GROUPS;this.faceData=fd.FACE;
 }
 async loadRig(){
  this.onProgress("Rig lädt …","MakeHuman default");
  const mod=await import("./rig-data.js?v=3.21.6");this.rig=mod.RIG;
