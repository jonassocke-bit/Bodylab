...
export class SilhouetteLab{
 ...
 async connect(){const state=this.panel.querySelector('#silSourceState'),info=this.panel.querySelector('#silSourceInfo');state.textContent='PRÜFT';info.innerHTML='<b>BodyM wird abgefragt …</b><span>S3-Listing ohne Login.</span>';try{const keys=await this.listS3();this.keys=keys;const csv=keys.filter(k=>/\.(csv|tsv)$/i.test(k));const imgs=keys.filter(k=>/\.(png|jpg|jpeg)$/i.test(k));info.innerHTML=`<b>Direktzugriff funktioniert</b><span>${keys.length} Schlüssel eingelesen · ${csv.length} Tabellen · ${imgs.length} Bilder im ersten Listing-Bereich. Metadaten werden gesucht …</span>`;state.textContent='ONLINE';await this.discoverPeople(csv,imgs)}catch(e){console.error(e);state.textContent='BLOCKIERT';info.innerHTML=`<b>Direktzugriff im Browser fehlgeschlagen</b><span>${esc(e.message)}. Das ist genau der Alpha-Test: falls S3-CORS auf iOS blockiert, bauen wir im nächsten Schritt eine accountfreie Zwischenquelle statt dich Dateien herunterladen zu lassen.</span>`}}
 async listS3(){let token='',keys=[];for(let page=0;page<6;page++){const u=S3+'/?list-type=2&max-keys=1000'+(token?'&continuation-token='+encodeURIComponent(token):'');const r=await fetch(u,{mode:'cors'});if(!r.ok)throw new Error('S3 HTTP '+r.status);const xml=new DOMParser().parseFromString(await r.text(),'application/xml');keys.push(...[...xml.querySelectorAll('Contents > Key')].map(x=>x.textContent));const next=xml.querySelector('NextContinuationToken')?.textContent;if(!next)break;token=next}if(!keys.length)throw new Error('Bucket antwortet, aber Listing ist leer');return keys}
 async discoverPeople(csvKeys,imgKeys){let rows=[];for(const k of csvKeys.slice(0,8)){try{const r=await fetch(S3+'/'+encodeURI(k),{mode:'cors'});if(!r.ok)continue;const t=await r.text();if(t.includes(',')&&/height|gender|chest|waist|hip/i.test(t)){rows.push(...csvParse(t).map(x=>({...x,__source:k})));if(rows.length>100)break}}catch{}}
 const byStem=new Map();for(const k of imgKeys){const low=k.toLowerCase(),stem=low.replace(/(front|frontal|side|lateral|profile)[^/]*\.(png|jpe?g)$/,'').replace(/\.(png|jpe?g)$/,'');const o=byStem.get(stem)||{};if(/front|frontal/.test(low))o.front=k;if(/side|lateral|profile/.test(low))o.side=k;byStem.set(stem,o)}
 const candidates=[...byStem.entries()].filter(([,v])=>v.front&&v.side);const people=[];for(let i=0;i<candidates.length;i++){const [stem,v]=candidates[i],meta=rows.find(r=>Object.values(r).some(x=>String(x).toLowerCase().includes(stem.split('/').pop())))||rows[i]||{};people.push({id:stem.split('/').pop()||('P'+i),front:v.front,side:v.side,meta,gender:normGender(first(meta,['gender','Gender','sex','Sex'])),height:num(first(meta,['height','Height','height_cm','stature'])),weight:num(first(meta,['weight','Weight','weight_kg']))})}
 this.people=this.pickTen(people);if(!this.people.length){this.panel.querySelector('#silPeopleState').textContent='KEINE PAARE';this.panel.querySelector('#silSourceInfo').innerHTML+=`<span>Listing erreichbar, aber Front/Side-Dateinamen konnten noch nicht automatisch gepaart werden. Gefundene Beispielpfade: ${esc(imgKeys.slice(0,6).join(' · '))}</span>`;return}this.panel.querySelector('#silPeopleState').textContent=this.people.length+'/10';await this.selectPerson(0)}
 ...
 async selectPerson(i){if(!this.people.length)return;this.personIndex=(i+this.people.length)%this.people.length;const p=this.people[this.personIndex];this.panel.querySelector('#silPersonTitle').textContent=`Person ${this.personIndex+1}/${this.people.length}`;this.panel.querySelector('#silPersonMeta').textContent=`${p.gender===0?'FRAU':p.gender===1?'MANN':'?'} · ${Number.isFinite(p.height)?fmt(p.height)+' cm':'Höhe ?'} · ${Number.isFinite(p.weight)?fmt(p.weight)+' kg':'Gewicht ?'} · ${p.id}`;try{this.ref.front=await this.loadMask(p.front,'#silFront');this.ref.side=await this.loadMask(p.side,'#silSide');this.renderMeasures();this.panel.querySelector('#silPeopleState').textContent='GELADEN'}catch(e){this.panel.querySelector('#silPeopleState').textContent='PIXEL BLOCK';this.panel.querySelector('#silSourceInfo').innerHTML+=`<span>Bild gefunden, aber Pixelzugriff scheitert: ${esc(e.message)}</span>`}}
