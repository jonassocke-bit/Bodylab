...
const STORE="bodylab_v312_hidden_model";
const HIDDEN=["shoulder","torso","waistBreadth","waistDepth","hipBreadth","neckBase"];
const OPTIMIZED=["shoulder","torso","waistBreadth","hipBreadth","neckBase"];
...
export class SolverV312{
 ...
 train(rows){
  ...
  this.model={build:"BODY LAB v3.12.0",createdAt:new Date().toISOString(),rows:usable.length,targets};
 ...
 residualVector(row,baselineShape=null){
  const cur=this.current(),res=[],keys=[],weights=[];
  for(const k of OPTIMIZED){
  ...
  if(baselineShape&&Number.isFinite(baselineShape.waistDepth)&&Number.isFinite(cur.waistDepth)){
   keys.push("waistDepth");
   res.push(refFromMesh("waistDepth",baselineShape.waistDepth)-refFromMesh("waistDepth",cur.waistDepth));
   weights.push(1.35);
  }
 ...
 morphCandidates(keys){
  ...
  for(const m of SENS.morphs){
   ...
   const waistDepthSpill=3*Math.abs(Number(m.all.waistDepth||0))/(m.step||.1),totalSpill=spill+waistDepthSpill;
   arr.push({...m,col,useful,spill:totalSpill,selectivity:useful/(.35+totalSpill)});
  }
 ...
 async correct(row,{passes=3}={}){
  ...
  const log=[],baselineShape=this.current();
  for(let pass=0;pass<passes;pass++){
   const {keys,res,weights}=this.residualVector(row,baselineShape);if(!keys.length)break;
  ...
  return {applied:true,log,final:this.residualVector(row,baselineShape)};
 }
 async baseline(row){
