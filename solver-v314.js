...
export class FinalSolverV314{
 constructor(engine,ui,lab,batch){
  this.engine=engine;this.ui=ui;this.lab=lab;this.batch=batch;this.model=null;
  try{this.model=JSON.parse(localStorage.getItem("bodylab_v314_final_model")||"null")}catch(e){}
 ...
 train(rows){
  ...
  try{localStorage.setItem("bodylab_v314_final_model",JSON.stringify(this.model))}catch(e){}
  return this.model;
 }
 ...
 repairState(){
  const h=this.engine.harnessBlindMetrics();
  return {shoulder:this.engine.shoulderBreadthCm(),torso:this.engine.shoulderToCrotchCm(),
   waistBreadth:h.waistBreadth,waistDepth:h.waistDepth,hipBreadth:h.hipBreadth,neckBase:h.neckBase};
 }
 repairObjective(state,baselineDepth,protectedState){
  if(!Number.isFinite(state.waistDepth)||!Number.isFinite(baselineDepth))return Infinity;
  const depth=Math.abs(state.waistDepth-baselineDepth);
  let spill=0;
  for(const k of ["shoulder","torso","waistBreadth","hipBreadth","neckBase"]){
   if(!Number.isFinite(state[k])||!Number.isFinite(protectedState[k]))continue;
   const d=Math.abs(state[k]-protectedState[k]);
   spill+=d<=.10?0:(d-.10)*(d-.10)*8;
  }
  return depth*depth+spill;
 }
 waistRepairCandidates(){
  const arr=[];
  for(const m of SENS.morphs){
   if(!(m.id in this.engine.directState))continue;
   const wd=Math.abs(Number(m.all.waistDepth||0))/(m.step||.1);
   if(wd<.08)continue;
   let spill=0;
   for(const k of ["shoulder","torso","waistBreadth","hipBreadth","neckBase"])
    spill+=Math.abs(Number(m.all[k]||0))/(m.step||.1);
   arr.push({...m,selectivity:wd/(.15+spill)});
  }
  arr.sort((a,b)=>b.selectivity-a.selectivity);
  return arr.slice(0,8);
 }
 async repairWaistDepth(row,baselineDepth){
  const protectedState=this.repairState(),beforeDepth=protectedState.waistDepth;
  if(!Number.isFinite(beforeDepth)||!Number.isFinite(baselineDepth))return {applied:false};
  let bestObj=this.repairObjective(protectedState,baselineDepth,protectedState);
  const log=[],candidates=this.waistRepairCandidates();

  for(const step of [.08,.04,.02]){
   let moved=false;
   for(const m of candidates){
    const id=m.id,cur=Number(this.engine.directState[id]||0);
    let bestV=cur,localBest=bestObj;
    for(const dir of [-1,1]){
     let v=Math.max(-1.25,Math.min(1.25,cur+dir*step));
     if(m.oneWay)v=Math.max(0,v);
     if(Math.abs(v-cur)<1e-6)continue;
     this.engine.directState[id]=v;this.engine.updateBody({normals:false,metrics:false});await this.relockCore5(row);
     const st=this.repairState(),obj=this.repairObjective(st,baselineDepth,protectedState);
     const safe=["shoulder","torso","waistBreadth","hipBreadth","neckBase"].every(k=>
      !Number.isFinite(st[k])||!Number.isFinite(protectedState[k])||Math.abs(st[k]-protectedState[k])<=.35);
     if(safe&&obj<localBest-1e-5){localBest=obj;bestV=v}
    }
    this.engine.directState[id]=bestV;this.engine.updateBody({normals:false,metrics:false});await this.relockCore5(row);
    if(Math.abs(bestV-cur)>.0001){moved=true;bestObj=localBest;log.push({id,target:m.target,from:cur,to:bestV})}
    else{this.engine.directState[id]=cur;this.engine.updateBody({normals:false,metrics:false});await this.relockCore5(row)}
   }
   if(!moved)break;
  }
  const after=this.repairState();
  if(Math.abs(after.waistDepth-baselineDepth)>=Math.abs(beforeDepth-baselineDepth)-.01){
   for(let i=log.length-1;i>=0;i--)this.engine.directState[log[i].id]=log[i].from;
   this.engine.updateBody({normals:false,metrics:false});await this.relockCore5(row);
   return {applied:false,beforeDepth,afterDepth:this.repairState().waistDepth,baselineDepth};
  }
  return {applied:true,beforeDepth,afterDepth:after.waistDepth,baselineDepth,log};
 }
 async finalCorrect(row){
  const baselineDepth=this.engine.harnessBlindMetrics().waistDepth;
  const v311=await this.correct(row);
  const repair=await this.repairWaistDepth(row,baselineDepth);
  return {v311,repair};
 }
