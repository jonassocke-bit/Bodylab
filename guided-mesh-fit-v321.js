...
export class GuidedMeshFitV321{
 ...
 <div class="generatorSectionTitle">MESH-FIT · V3.30.0</div>
 ...
 <section class="fcStep"><div class="fcStepHead"><span>9</span><div><b>V3.26 Zwei-Stufen-Fit</b><small>Stage 1 normal · Stage 2 Overdrive nur bei Bedarf</small></div><strong id="fit26Status">BEREIT</strong></div>
 <div class="generatorIntro">
  <b>Stage 1:</b> d8 und Brust-Morphs bleiben innerhalb ±100 %.
  <b>Stage 2:</b> Nur wenn die weibliche Brusttiefe danach noch deutlich zu klein ist,
  dürfen die nachweislich hilfreichen Tiefen-/Brust-Projektionsregler kontrolliert über 100 % hinaus.
  Andere Maße bleiben geschützt. Jeder Overdrive-Wert wird im Ergebnis protokolliert.
 </div>
 <div class="generatorActions"><button id="fit26Run" class="primary">100 Personen V3.26 fitten</button><button id="fit26Abort">Abbrechen</button></div>
 <div id="fit26Progress" class="batchProgressRich hidden"><div class="batchProgressTop"><b>V3.26 Two-Stage Fit</b><span id="fit26Pct">0%</span></div><div class="batchProgressTrack"><div id="fit26Bar"></div></div><div class="batchProgressMeta"><span id="fit26Count">0 / 100</span><span id="fit26Eta">Restzeit wird geschätzt …</span></div></div>
 <div id="fit26Result" class="calResults"></div>
 </section>
 <section class="fcStep"><div class="fcStepHead"><span>10</span><div><b>V3.27 Hierarchischer Fit</b><small>A: Basis schützen · B: Tiefe priorisieren · C: Kollateralschäden reparieren</small></div><strong id="fit27Status">BEREIT</strong></div>
 <div class="generatorIntro">
  <b>Stage A:</b> stabile Ausgangsgeometrie festhalten.
  <b>Stage B:</b> Brusttiefe konsequent auf Ziel bringen; Overdrive darf bis 200 % gehen,
  aber nur solange zusätzliche Übersteuerung die echte Messabweichung weiter reduziert.
  <b>Stage C:</b> Brustbreite, Taille, Hüfte, Torso, Schulter und Halsbasis anschließend gezielt reparieren,
  ohne die erreichte Brusttiefe wieder deutlich zu verlieren.
 </div>
 <div class="generatorActions"><button id="fit27Run" class="primary">100 Personen V3.27 fitten</button><button id="fit27Abort">Abbrechen</button></div>
 <div id="fit27Progress" class="batchProgressRich hidden"><div class="batchProgressTop"><b>V3.27 Hierarchical Fit</b><span id="fit27Pct">0%</span></div><div class="batchProgressTrack"><div id="fit27Bar"></div></div><div class="batchProgressMeta"><span id="fit27Count">0 / 100</span><span id="fit27Eta">Restzeit wird geschätzt …</span></div></div>
 <div id="fit27Result" class="calResults"></div>
 </section>
 <section class="fcStep"><div class="fcStepHead"><span>11</span><div><b>V3.29 Finaler Multi-Stage Repair</b><small>resumable · Live-Mannequin · Checkpoint nach jeder Person</small></div><strong id="fit29Status">BEREIT</strong></div>
 <div class="generatorIntro"><b>Brusttiefe aus V3.27 bleibt geschützt.</b> Danach werden Hüfte, Taille und Torso mehrstufig repariert. Jede Stufe startet bis 100 % und darf nur bei weiter sinkendem realen Mesh-Fehler adaptiv bis 200 % gehen. Nach jeder fertigen Person wird der komplette Teststand gespeichert.</div>
 <label class="batchInfo"><input id="fit29Live" type="checkbox" checked> Live-Ansicht: Morph-Schritte am Mannequin sichtbar (etwas langsamer)</label>
 <div class="generatorActions"><button id="fit29Run" class="primary">Start / Fortsetzen</button><button id="fit29Pause">Pausieren</button><button id="fit29Hide">Menü schließen · Live ansehen</button><button id="fit29Reset">Teststand verwerfen</button></div>
 <div id="fit29Progress" class="batchProgressRich hidden"><div class="batchProgressTop"><b>V3.29 Multi-Stage Repair</b><span id="fit29Pct">0%</span></div><div class="batchProgressTrack"><div id="fit29Bar"></div></div><div class="batchProgressMeta"><span id="fit29Count">0 / 100</span><span id="fit29Eta">bereit</span></div></div>
 <div id="fit29Resume" class="batchInfo"></div><div id="fit29Result" class="calResults"></div>
 </section>`;this.panel.appendChild(h);this.ensureLiveHud();this.syncV329Checkpoint()}
 bind(){this.panel.querySelector('#mdRun').onclick=()=>this.run();this.panel.querySelector('#mdAbort').onclick=()=>{this.abort=true};this.panel.querySelector('#objRun').onclick=()=>this.runObjective();this.panel.querySelector('#objAbort').onclick=()=>{this.abort=true};this.panel.querySelector('#auditRun').onclick=()=>this.runTargetAudit();this.panel.querySelector('#auditAbort').onclick=()=>{this.abort=true};this.panel.querySelector('#limitRun').onclick=()=>this.runLimitGenderAudit();this.panel.querySelector('#limitAbort').onclick=()=>{this.abort=true};this.panel.querySelector('#fit23Run').onclick=()=>this.runV323Fit();this.panel.querySelector('#fit23Abort').onclick=()=>{this.abort=true};this.panel.querySelector('#fit24Run').onclick=()=>this.runV324Fit();this.panel.querySelector('#fit24Abort').onclick=()=>{this.abort=true};this.panel.querySelector('#fit25Run').onclick=()=>this.runV325Fit();this.panel.querySelector('#fit25Abort').onclick=()=>{this.abort=true};this.panel.querySelector('#fit26Run').onclick=()=>this.runV326Fit();this.panel.querySelector('#fit26Abort').onclick=()=>{this.abort=true};this.panel.querySelector('#fit27Run').onclick=()=>this.runV327Fit();this.panel.querySelector('#fit27Abort').onclick=()=>{this.abort=true};this.panel.querySelector('#fit29Run').onclick=()=>this.runV329Fit();this.panel.querySelector('#fit29Pause').onclick=()=>{this.pauseV329=true};this.panel.querySelector('#fit29Hide').onclick=()=>this.panel.classList.add('hidden');this.panel.querySelector('#fit29Reset').onclick=()=>this.resetV329Checkpoint()}
 ...
 async scanD8Overdrive(r,targetDepth,tgt,maxAbs=1.6){
  const c=this.findMorphByTarget('torso-scale-depth');if(!c)return null;
  let best={score:Infinity,v:Number(this.engine.directState[c.id]||0),state:this.current()};
  const test=(v)=>Math.max(-maxAbs,Math.min(maxAbs,Math.round(v*100)/100));
  for(let v=-maxAbs;v<=maxAbs+.0001;v+=.10){
   this.engine.directState[c.id]=test(v);await this.refreshLock(r);
   const cur=this.current(),err=Math.abs(cur.chestDepth-targetDepth),protect=this.protectedObjective(cur,tgt);
   const over=Math.max(0,Math.abs(this.engine.directState[c.id])-1);
   const score=err*err/100+protect*.55+over*over*.06;
   if(score<best.score)best={score,v:this.engine.directState[c.id],state:cur};
  }
  this.engine.directState[c.id]=best.v;await this.refreshLock(r);
  return {id:c.id,target:c.target,value:best.v,state:this.current()};
 }
 async optimizeFemaleProjectionOverdrive(r,tgt,baselineProtected,maxAbs=1.8){
  const controls=this.localBreastControls();
  let best={score:Infinity,size:this.engine.state.breastSize,firm:this.engine.state.breastFirmness};
  for(const size of [0,.25,.5,.75,1.0,1.2,1.4,1.6]){
   for(const firm of [.25,.5,.75,1.0]){
    this.engine.state.breastSize=size;
    this.engine.state.breastFirmness=firm;
    for(const c of controls)this.engine.directState[c.id]=0;
    await this.refreshLock(r);
    const q=await this.scoreFemaleLocalState(r,tgt,baselineProtected);
    const over=Math.max(0,size-1);
    const score=q.score+over*over*.03;
    if(score<best.score)best={score,size,firm};
   }
  }
  this.engine.state.breastSize=best.size;this.engine.state.breastFirmness=best.firm;
  for(const c of controls)this.engine.directState[c.id]=0;
  ...
  for(const step of [.30,.15,.07,.03]){
   for(const c of controls){
    const curV=Number(this.engine.directState[c.id]||0);
    let local={v:curV,score:(await this.scoreFemaleLocalState(r,tgt,baselineProtected)).score};
    for(const dir of [-1,1]){
     const v=Math.max(-maxAbs,Math.min(maxAbs,curV+dir*step));
     this.engine.directState[c.id]=v;await this.refreshLock(r);
     const q=await this.scoreFemaleLocalState(r,tgt,baselineProtected);
     const over=Math.max(0,Math.abs(v)-1);
     const score=q.score+over*over*.025;
     if(score<local.score)local={v,score};
    }
    this.engine.directState[c.id]=local.v;await this.refreshLock(r);
   }
 ...
 async femaleV326Fit(r,tgt){
  ...
  // Stage 1: normal range only
  const d8stage1=await this.scanD8ToTarget(r,thoraxTarget,tgt);
  const protectedBase=this.current();
  const local1=await this.optimizeFemaleProjectionLocal(r,tgt,protectedBase);
  const stage1State=this.current();
  const stage1Err=Math.abs(stage1State.chestDepth-totalTarget);

  if(stage1Err<=1.25){
   return {stage:1,thoraxTarget,totalTarget,d8:d8stage1?.value,breastSize:local1.breastSize,breastFirmness:local1.breastFirmness,local:local1.local,state:stage1State};
  }

  // Stage 2: controlled overdrive. Start from neutral breast projection again.
  this.engine.state.breastSize=0;this.engine.state.breastFirmness=.5;
  for(const c of controls)this.engine.directState[c.id]=0;
  await this.refreshLock(r);

  const d8stage2=await this.scanD8Overdrive(r,thoraxTarget,tgt,1.6);
  const protectedStage2=this.current();
  const local2=await this.optimizeFemaleProjectionOverdrive(r,tgt,protectedStage2,1.8);
  const stage2State=this.current();

  // Choose stage 2 only if it materially improves chest depth without excessive collateral damage.
  const q1=this.protectedObjective(stage1State,tgt,protectedBase);
  const q2=this.protectedObjective(stage2State,tgt,protectedStage2);
  const e1=Math.abs(stage1State.chestDepth-totalTarget),e2=Math.abs(stage2State.chestDepth-totalTarget);
  if(e2<e1-.35 && q2<=q1+.02){
   return {stage:2,thoraxTarget,totalTarget,d8:d8stage2?.value,breastSize:local2.breastSize,breastFirmness:local2.breastFirmness,local:local2.local,state:stage2State};
  }

  // restore Stage 1 state if Stage 2 does not earn its extra deformation
  await this.baseline(r);
  if(solver?.correct)await solver.correct(r);
  await this.refreshLock(r);
  this.engine.state.breastSize=0;this.engine.state.breastFirmness=.5;
  for(const c of controls)this.engine.directState[c.id]=0;
  await this.refreshLock(r);
  await this.scanD8ToTarget(r,thoraxTarget,tgt);
  await this.optimizeFemaleProjectionLocal(r,tgt,this.current());
  return {stage:1,thoraxTarget,totalTarget,d8:d8stage1?.value,breastSize:this.engine.state.breastSize,breastFirmness:this.engine.state.breastFirmness,local:Object.fromEntries(controls.map(c=>[c.target,Number(this.engine.directState[c.id]||0)])),state:this.current()};
 }
 async fitV326Person(r){
  await this.baseline(r);
  const tgt=this.target(r);
  if(r.gender===0)return {gender:'Frauen',...(await this.femaleV326Fit(r,tgt))};
  return {gender:'Männer',stage:1,...(await this.maleV325Fit(r,tgt))};
 }

 repairControlsForMeasure(key){
  const map={
   chestBreadth:[/torso-scale-horiz/i,/breast-dist/i,/breast-point/i],
   waistBreadth:[/waist/i,/stomach/i,/torso-scale-horiz/i],
   waistDepth:[/waist/i,/stomach/i,/torso-scale-depth/i],
   hipBreadth:[/hip-scale-horiz/i,/pelvis/i,/butt/i],
   shoulder:[/shoulder/i,/torso-scale-horiz/i],
   torso:[/torso-scale-vert/i,/torso-trans/i],
   neckBase:[/neck-scale/i,/neck-trans/i]
  };
  const pats=map[key]||[],out=[],seen=new Set();
  for(const group of this.engine.groups||[])for(const c of group.controls||[]){
   const semantic=`${group.id||""} ${c.group||""} ${c.target||""}`;
   if(!c?.id||seen.has(c.id)||!pats.some(p=>p.test(semantic)))continue;
   if(/face|eye|nose|mouth|ear/i.test(semantic))continue;
   seen.add(c.id);out.push(c);
  }
  return out.slice(0,12);
 }
 collateralScore(cur,tgt,depthAnchor){
  const w={chestBreadth:2.5,waistBreadth:2.0,waistDepth:2.0,hipBreadth:2.2,shoulder:1.5,torso:1.7,neckBase:1.5};
  let s=0,n=0;
  for(const [k,ww] of Object.entries(w)){
   if(!Number.isFinite(cur[k])||!Number.isFinite(tgt[k]))continue;
   const e=(cur[k]-tgt[k])/Math.max(10,Math.abs(tgt[k]));s+=ww*e*e;n+=ww;
  }
  if(Number.isFinite(cur.chestDepth)&&Number.isFinite(depthAnchor)){
   const drift=Math.abs(cur.chestDepth-depthAnchor);
   if(drift>.20)s+=(drift-.20)*(drift-.20)*25;
  }
  return n?s/n:Infinity;
 }
 async maximizeChestDepthFemale(r,tgt){
  const solver=this.calibration?.workflow?.solver;
  if(solver?.correct)await solver.correct(r);
  await this.refreshLock(r);

  let thoraxTarget=this.predictMaleThorax(r);
  thoraxTarget=Math.min(tgt.chestDepth,Math.max(tgt.chestDepth*.58,thoraxTarget));
  ...
  const d8=this.findMorphByTarget('torso-scale-depth');
  let bestD8={v:Number(this.engine.directState[d8.id]||0),err:Infinity};
  let worsening=0,lastErr=Infinity;
  for(let v=-1;v<=2.0001;v+=.10){
   this.engine.directState[d8.id]=Math.round(v*100)/100;await this.refreshLock(r);
   const err=Math.abs(this.current().chestDepth-thoraxTarget);
   if(err<bestD8.err-.01)bestD8={v:this.engine.directState[d8.id],err};
   if(v>=1){
    if(err>lastErr+.03)worsening++; else worsening=0;
    if(worsening>=2)break;
   }
   lastErr=err;
  }
  this.engine.directState[d8.id]=bestD8.v;await this.refreshLock(r);

  let best={err:Math.abs(this.current().chestDepth-tgt.chestDepth),size:0,firm:.5};
  for(const size of [0,.25,.5,.75,1,1.25,1.5,1.75,2]){
   for(const firm of [.25,.5,.75,1]){
    this.engine.state.breastSize=size;this.engine.state.breastFirmness=firm;
    for(const c of controls)this.engine.directState[c.id]=0;
    await this.refreshLock(r);
    const err=Math.abs(this.current().chestDepth-tgt.chestDepth);
    if(err<best.err-.01)best={err,size,firm};
   }
  }
  this.engine.state.breastSize=best.size;this.engine.state.breastFirmness=best.firm;
  for(const c of controls)this.engine.directState[c.id]=0;
  ...
  for(const step of [.35,.18,.08,.04]){
   for(const c of controls){
    const curV=Number(this.engine.directState[c.id]||0);
    let local={v:curV,err:Math.abs(this.current().chestDepth-tgt.chestDepth)};
    for(const dir of [-1,1]){
     const v=Math.max(-2,Math.min(2,curV+dir*step));
     this.engine.directState[c.id]=v;await this.refreshLock(r);
     const err=Math.abs(this.current().chestDepth-tgt.chestDepth);
     if(err<local.err-.005)local={v,err};
    }
    this.engine.directState[c.id]=local.v;await this.refreshLock(r);
   }
  }
  return {thoraxTarget,d8:Number(this.engine.directState[d8.id]||0),breastSize:this.engine.state.breastSize,breastFirmness:this.engine.state.breastFirmness,local:Object.fromEntries(controls.map(c=>[c.target,Number(this.engine.directState[c.id]||0)])),state:this.current()};
 }
 async repairCollateral(r,tgt,depthAnchor){
  const keys=['chestBreadth','waistBreadth','waistDepth','hipBreadth','shoulder','torso','neckBase'];
  for(const key of keys){
   const controls=this.repairControlsForMeasure(key);
   if(!controls.length)continue;
   for(const step of [.18,.08,.04]){
    let moved=false;
    for(const c of controls){
     const curV=Number(this.engine.directState[c.id]||0);
     let best={v:curV,score:this.collateralScore(this.current(),tgt,depthAnchor)};
     for(const dir of [-1,1]){
      const v=Math.max(-2,Math.min(2,curV+dir*step));
      this.engine.directState[c.id]=v;await this.refreshLock(r);
      const score=this.collateralScore(this.current(),tgt,depthAnchor);
      if(score<best.score-1e-5)best={v,score};
     }
     this.engine.directState[c.id]=best.v;await this.refreshLock(r);
     if(Math.abs(best.v-curV)>.0001)moved=true;
    }
    if(!moved)break;
   }
  }
 ...
 async fitV327Person(r){
  await this.baseline(r);
  const tgt=this.target(r);
  if(r.gender===1)return {gender:'Männer',stage:'M',...(await this.maleV325Fit(r,tgt))};

  const solver=this.calibration?.workflow?.solver;
  if(solver?.correct)await solver.correct(r);
  await this.refreshLock(r);
  const depthFit=await this.maximizeChestDepthFemale(r,tgt);
  const depthAnchor=this.current().chestDepth;
...
ensureLiveHud(){
 ...
 setV329Hud(text,show=true){const d=document.getElementById('fit29Hud');if(d){d.textContent=text;d.style.display=show?'block':'none'}}
 async liveYield(label){this.setV329Hud(label,true);if(this.panel.querySelector('#fit29Live')?.checked)await new Promise(r=>setTimeout(r,80));else await new Promise(r=>setTimeout(r,0))}
 v329Key(){return 'bodylab_v329_checkpoint'}
 loadV329Checkpoint(){try{return JSON.parse(localStorage.getItem(this.v329Key())||'null')}catch(e){return null}}
 saveV329Checkpoint(cp){localStorage.setItem(this.v329Key(),JSON.stringify(cp));this.syncV329Checkpoint()}
 resetV329Checkpoint(){localStorage.removeItem(this.v329Key());this.pauseV329=false;this.setV329Hud('',false);this.syncV329Checkpoint();this.panel.querySelector('#fit29Result').innerHTML=''}
 syncV329Checkpoint(){const e=this.panel.querySelector('#fit29Resume');if(!e)return;const cp=this.loadV329Checkpoint();e.innerHTML=cp?`<b>Gespeicherter Teststand</b><span>${cp.next||0} / ${cp.total||100} Personen abgeschlossen${cp.done?' · fertig':''}</span>`:'<b>Noch kein gespeicherter Lauf.</b><span>Startet bei Person 1.</span>'}
 repair329Controls(key){return this.repairControlsForMeasure(key)}
 score329(cur,tgt,focus,anchors){
  const w={hipBreadth:3,waistBreadth:3,waistDepth:2.5,torso:2.8,chestBreadth:1.8,shoulder:1.4,neckBase:1.4};let q=0,n=0;
  for(const [k,ww0] of Object.entries(w)){if(!Number.isFinite(cur[k])||!Number.isFinite(tgt[k]))continue;const ww=k===focus?ww0*5:ww0,e=(cur[k]-tgt[k])/Math.max(10,Math.abs(tgt[k]));q+=ww*e*e;n+=ww}
  for(const [k,a] of Object.entries(anchors||{})){if(!Number.isFinite(cur[k])||!Number.isFinite(a))continue;const tol=k==='chestDepth'?.18:.22,d=Math.max(0,Math.abs(cur[k]-a)-tol);q+=28*d*d}return n?q/n:Infinity
 }
 async adaptive329Stage(r,tgt,key,anchors,personNo){
  const cs=this.repair329Controls(key);let maxAbs=0,moves=0;if(!cs.length)return {key,maxAbs,moves};
  for(const step of [.18,.08,.035])for(const c of cs){
   const start=Number(this.engine.directState[c.id]||0);let best={v:start,score:this.score329(this.current(),tgt,key,anchors)};
   for(const dir of [-1,1]){const v=Math.max(-1,Math.min(1,start+dir*step));this.engine.directState[c.id]=v;await this.refreshLock(r);await this.liveYield(`Person ${personNo}/100 · ${key} · ${c.target} ${(v*100).toFixed(0)}%`);const sc=this.score329(this.current(),tgt,key,anchors);if(sc<best.score-1e-6)best={v,score:sc}}
   this.engine.directState[c.id]=best.v;await this.refreshLock(r);if(Math.abs(best.v-start)>1e-5)moves++;maxAbs=Math.max(maxAbs,Math.abs(best.v));
   if(Math.abs(best.v)>=.999){const dir=Math.sign(best.v)||1;let last=best;for(const lim of [1.2,1.5,1.75,2]){this.engine.directState[c.id]=dir*lim;await this.refreshLock(r);await this.liveYield(`Person ${personNo}/100 · ${key} · Overdrive ${(lim*100).toFixed(0)}%`);const sc=this.score329(this.current(),tgt,key,anchors);if(sc<last.score-1e-6){last={v:dir*lim,score:sc};maxAbs=Math.max(maxAbs,lim);moves++}else{this.engine.directState[c.id]=last.v;await this.refreshLock(r);break}}}
  }
  return {key,maxAbs,moves}
 }
 async fitV329Person(r,personNo){
  const fit=await this.fitV327Person(r);const tgt=this.target(r);if(r.gender!==0)return {fit,stages:[]};
  const anchors={chestDepth:this.current().chestDepth},stages=[];
 ...
 renderV329(cp){
  ...
  this.panel.querySelector('#fit29Result').innerHTML=`<div class="optimizerHero ${good?'hit':'miss'}"><small>V3.29 · ${cp.next} PERSONEN</small><strong>${cp.done?(good?'BEREIT FÜR FINAL-HOLDOUT':'NOCH NICHT EINFRIEREN'):'PAUSIERT / ZWISCHENSTAND'}</strong><span>Gesamt-MAE ${F(B)} → <b>${F(A)} cm</b> · Brusttiefe ${F(cdB)} → <b>${F(cdA)} cm</b></span></div><div class="batchMeasureMatrix"><b>Geschlecht</b><span>Frauen gesamt ${F(mean(cp.fb))} → <strong>${F(mean(cp.fa))}</strong> · Brusttiefe ${F(mean(cp.fcdB))} → <strong>${F(mean(cp.fcdA))}</strong></span><span>Männer gesamt ${F(mean(cp.mb))} → <strong>${F(mean(cp.ma))}</strong> · Brusttiefe ${F(mean(cp.mcdB))} → <strong>${F(mean(cp.mcdA))}</strong></span></div><div class="batchMeasureMatrix"><b>Einzelmaße</b>${TARGETS.map(([k,l])=>`<span>${l}: ${F(mean(cp.perB[k]))} → <strong>${F(mean(cp.perA[k]))} cm</strong></span>`).join('')}</div><div class="batchInfo"><b>Overdrive >100%</b><span>${Object.keys(cp.over).length?Object.entries(cp.over).map(([k,n])=>`${k}: ${n}×`).join(' · '):'—'}</span></div><div class="batchInfo"><b>Schutzprüfung</b><span>${regress.length?`Schlechter >0,20 cm: ${regress.join(', ')}`:'Keine relevante Regression.'}</span></div>`
 }
 async runV329Fit(){
  ...
  try{for(let i=cp.next;i<rows.length;i++){
   ...
   cp.next=i+1;cp.elapsed+=(performance.now()-t0)/1000;this.saveV329Checkpoint(cp);this.renderV329(cp);const sec=(performance.now()-t0)/1000;if(sec>.01&&sec<900)samples.push(sec);if(samples.length>10)samples.shift();const pct=100*cp.next/rows.length,med=samples.length?[...samples].sort((a,b)=>a-b)[Math.floor(samples.length/2)]:NaN;this.panel.querySelector('#fit29Pct').textContent=pct.toFixed(0)+'%';this.panel.querySelector('#fit29Bar').style.width=pct+'%';this.panel.querySelector('#fit29Count').textContent=`${cp.next} / ${rows.length}`;this.panel.querySelector('#fit29Eta').textContent=Number.isFinite(med)?`ca. ${Math.round(med*(rows.length-cp.next)/60)} min verbleibend`:'Checkpoint gespeichert';this.setV329Hud(`Person ${cp.next}/${rows.length} abgeschlossen · Checkpoint gespeichert`,true);
   if(this.pauseV329){this.panel.querySelector('#fit29Status').textContent='PAUSIERT';return}
  }cp.done=true;this.saveV329Checkpoint(cp);this.renderV329(cp);this.panel.querySelector('#fit29Status').textContent='ERLEDIGT';this.setV329Hud('V3.29 · 100/100 abgeschlossen',true)}finally{this.engine.restore(snap);this.ui.sync();this.engine.computeMetrics()}
 }
 ...
 async runV327Fit(){
  ...
  const prog=this.panel.querySelector('#fit27Progress');prog.classList.remove('hidden');this.panel.querySelector('#fit27Status').textContent='LÄUFT';this.abort=false;
  const thoraxModel=this.trainMaleThoraxModel();

  try{
   for(let i=0;i<rows.length;i++){
    if(this.abort)break;
    const t0=performance.now(),r=rows[i];await this.baseline(r);
    ...
    const fit=await this.fitV327Person(r),aMesh=this.current(),aErr=[];
    ...
    if(details.length<8&&r.gender===0)details.push({raw:rawCD,target:this.target(r).chestDepth,mesh:aMesh.chestDepth,anchor:fit.depthAnchor,thorax:fit.thoraxTarget,d8:fit.d8,size:fit.breastSize,local:fit.local});

    const sec=(performance.now()-t0)/1000;if(sec>.01&&sec<300)samples.push(sec);if(samples.length>15)samples.shift();
    const done=i+1,pct=100*done/rows.length,med=samples.length?[...samples].sort((a,b)=>a-b)[Math.floor(samples.length/2)]:NaN;
    this.panel.querySelector('#fit27Pct').textContent=pct.toFixed(0)+'%';this.panel.querySelector('#fit27Bar').style.width=pct+'%';this.panel.querySelector('#fit27Count').textContent=`${done} / ${rows.length}`;
    this.panel.querySelector('#fit27Eta').textContent=Number.isFinite(med)&&done>2?`ca. ${Math.max(0,Math.round(med*(rows.length-done)/60))} min verbleibend`:'Restzeit wird geschätzt …';
    await new Promise(q=>setTimeout(q,0));
   }
   ...
   const regress=protectedKeys.filter(k=>{const b=mean(perBefore[k]),a=mean(perAfter[k]);return Number.isFinite(a)&&Number.isFinite(b)&&a>b+.20});
   const good=Number.isFinite(A)&&A<B-.15&&Number.isFinite(femA)&&femA<=2.0&&Number.isFinite(maleA)&&maleA<=1.4&&regress.length<=1;
   const localFmt=o=>o?Object.entries(o).filter(([,v])=>Math.abs(v)>.01).map(([k,v])=>`${k} ${v>=0?'+':''}${fmt2(v)}`).join(' · '):'—';

   this.panel.querySelector('#fit27Result').innerHTML=
    `<div class="optimizerHero ${good?'hit':'miss'}"><small>V3.27 · ${after.length} PERSONEN · THORAX-MODELL n=${thoraxModel.n}</small><strong>${good?'BEREIT FÜR FINAL-HOLDOUT':'NOCH NICHT EINFRIEREN'}</strong><span>Gesamt-MAE ${fmt2(B)} → <b>${fmt2(A)} cm</b> · Brusttiefe ${fmt2(cdB)} → <b>${fmt2(cdA)} cm</b></span></div>
     ...
     <div class="batchMeasureMatrix"><b>Einzelmaße · MAE vorher → V3.27</b>${TARGETS.map(([k,label])=>`<span>${label}: ${fmt2(mean(perBefore[k]))} → <strong>${fmt2(mean(perAfter[k]))} cm</strong></span>`).join('')}</div>
     <div class="batchInfo"><b>Schutzprüfung</b><span>${regress.length?`Schlechter >0,20 cm: ${regress.join(', ')}`:'Alle geschützten Maße bleiben stabil.'}</span></div>
     <div class="batchMeasureMatrix"><b>8 Frauen-Beispiele · Tiefe → Reparatur</b>${details.map((x,i)=>`<span>F${i+1}: ANSUR ${fmt2(x.raw)} · Solver ${fmt2(x.target)} · Tiefen-Anker ${fmt2(x.anchor)} · final ${fmt2(x.mesh)} · Thorax ${fmt2(x.thorax)} · d8 ${fmt2(x.d8)} · BreastSize ${fmt2(x.size)} · ${localFmt(x.local)}</span>`).join('')}</div>
     <div class="batchInfo"><b>Entscheidung</b><span>${good?'Nur noch Final-Holdout. Danach einfrieren und Kalibrierungsphase beenden.':'Kein neuer allgemeiner Trainingszyklus. Nur den konkret ausgewiesenen Restengpass korrigieren.'}</span></div>`;
   this.panel.querySelector('#fit27Status').textContent=this.abort?'PAUSIERT':'ERLEDIGT';
  }finally{this.engine.restore(snap);this.ui.sync();this.engine.computeMetrics()}
 }
 ...
 async runV326Fit(){
  ...
  const snap=this.engine.snapshot(),samples=[],before=[],after=[],femaleBefore=[],femaleAfter=[],maleBefore=[],maleAfter=[];
  const perBefore=Object.fromEntries(TARGETS.map(([k])=>[k,[]])),perAfter=Object.fromEntries(TARGETS.map(([k])=>[k,[]]));
  const sexChest={Frauen:{b:[],a:[]},Männer:{b:[],a:[]}},details=[];let stage2Count=0;
  const prog=this.panel.querySelector('#fit26Progress');prog.classList.remove('hidden');this.panel.querySelector('#fit26Status').textContent='LÄUFT';this.abort=false;
  const thoraxModel=this.trainMaleThoraxModel();
  ...
  try{
   for(let i=0;i<rows.length;i++){
    ...
    const fit=await this.fitV326Person(r);if(fit.stage===2)stage2Count++;
    ...
    if(details.length<8&&r.gender===0)details.push({stage:fit.stage,raw:rawCD,target:this.target(r).chestDepth,thorax:fit.thoraxTarget,mesh:aMesh.chestDepth,d8:fit.d8,size:fit.breastSize,local:fit.local});

    const sec=(performance.now()-t0)/1000;if(sec>.01&&sec<300)samples.push(sec);if(samples.length>15)samples.shift();
    const done=i+1,pct=100*done/rows.length,med=samples.length?[...samples].sort((a,b)=>a-b)[Math.floor(samples.length/2)]:NaN;
    this.panel.querySelector('#fit26Pct').textContent=pct.toFixed(0)+'%';this.panel.querySelector('#fit26Bar').style.width=pct+'%';this.panel.querySelector('#fit26Count').textContent=`${done} / ${rows.length}`;
    this.panel.querySelector('#fit26Eta').textContent=Number.isFinite(med)&&done>2?`ca. ${Math.max(0,Math.round(med*(rows.length-done)/60))} min verbleibend`:'Restzeit wird geschätzt …';
    await new Promise(q=>setTimeout(q,0));
   }
   ...
   const femB=mean(sexChest.Frauen.b),femA=mean(sexChest.Frauen.a),maleB=mean(sexChest.Männer.b),maleA=mean(sexChest.Männer.a);
   const protectedKeys=['chestBreadth','waistBreadth','waistDepth','hipBreadth','shoulder','torso','neckBase'];
   const regress=protectedKeys.filter(k=>{const b=mean(perBefore[k]),a=mean(perAfter[k]);return Number.isFinite(a)&&Number.isFinite(b)&&a>b+.25});
   const good=Number.isFinite(A)&&A<B-.10&&Number.isFinite(femA)&&femA<=2.2&&Number.isFinite(maleA)&&maleA<=1.4&&regress.length<=1;
   const localFmt=o=>o?Object.entries(o).filter(([,v])=>Math.abs(v)>.01).map(([k,v])=>`${k} ${v>=0?'+':''}${fmt2(v)}`).join(' · '):'—';

   this.panel.querySelector('#fit26Result').innerHTML=
    `<div class="optimizerHero ${good?'hit':'miss'}"><small>V3.26 · ${after.length} PERSONEN · STAGE-2 bei ${stage2Count}</small><strong>${good?'BEREIT FÜR FINAL-HOLDOUT':'NOCH NICHT EINFRIEREN'}</strong><span>Gesamt-MAE ${fmt2(B)} → <b>${fmt2(A)} cm</b> · Brusttiefe ${fmt2(cdB)} → <b>${fmt2(cdA)} cm</b></span></div>
     <div class="batchMeasureMatrix"><b>Brusttiefe getrennt</b><span>Frauen: ${fmt2(femB)} → <strong>${fmt2(femA)} cm</strong></span><span>Männer: ${fmt2(maleB)} → <strong>${fmt2(maleA)} cm</strong></span></div>
     <div class="batchMeasureMatrix"><b>Gesamtfehler je Geschlecht</b><span>Frauen: ${fmt2(mean(femaleBefore))} → <strong>${fmt2(mean(femaleAfter))} cm</strong></span><span>Männer: ${fmt2(mean(maleBefore))} → <strong>${fmt2(mean(maleAfter))} cm</strong></span></div>
     <div class="batchMeasureMatrix"><b>Einzelmaße · MAE vorher → V3.26</b>${TARGETS.map(([k,label])=>`<span>${label}: ${fmt2(mean(perBefore[k]))} → <strong>${fmt2(mean(perAfter[k]))} cm</strong></span>`).join('')}</div>
     ...
     <div class="batchMeasureMatrix"><b>8 Frauen-Beispiele · Stage/Overdrive</b>${details.map((x,i)=>`<span>F${i+1}: Stage ${x.stage} · ANSUR ${fmt2(x.raw)} · Solver ${fmt2(x.target)} · Thorax ${fmt2(x.thorax)} · Mesh ${fmt2(x.mesh)} · d8 ${fmt2(x.d8)} · BreastSize ${fmt2(x.size)} · ${localFmt(x.local)}</span>`).join('')}</div>`;
   this.panel.querySelector('#fit26Status').textContent=this.abort?'PAUSIERT':'ERLEDIGT';
  }finally{this.engine.restore(snap);this.ui.sync();this.engine.computeMetrics()}
 }
