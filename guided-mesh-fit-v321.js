...
export class GuidedMeshFitV321{
 ...
 <div class="generatorSectionTitle">MESH-FIT · V3.28.0</div>
 ...
 <section class="fcStep"><div class="fcStepHead"><span>11</span><div><b>V3.28 Multi-Stage Repair</b><small>Brust schützen → Hüfte → Taille → Torso → Rest → Feinschliff</small></div><strong id="fit28Status">BEREIT</strong></div>
 <div class="generatorIntro"><b>Adaptiver Overdrive:</b> Jede Stufe beginnt bis 100 %. Nur wenn der echte Mesh-Fehler am Limit weiter sinkt, wird auf 120 / 150 / 175 / maximal 200 % erweitert. Nach jedem Versuch wird das reale Mesh neu gemessen; Brusttiefe und bereits reparierte Maße bleiben geschützt.</div>
 <div class="generatorActions"><button id="fit28Run" class="primary">100 Personen V3.28 fitten</button><button id="fit28Abort">Abbrechen</button></div>
 <div id="fit28Progress" class="batchProgressRich hidden"><div class="batchProgressTop"><b>V3.28 Multi-Stage Repair</b><span id="fit28Pct">0%</span></div><div class="batchProgressTrack"><div id="fit28Bar"></div></div><div class="batchProgressMeta"><span id="fit28Count">0 / 100</span><span id="fit28Eta">Restzeit wird geschätzt …</span></div></div>
 <div id="fit28Result" class="calResults"></div>
 </section>`;this.panel.appendChild(h)}
 bind(){this.panel.querySelector('#mdRun').onclick=()=>this.run();this.panel.querySelector('#mdAbort').onclick=()=>{this.abort=true};this.panel.querySelector('#objRun').onclick=()=>this.runObjective();this.panel.querySelector('#objAbort').onclick=()=>{this.abort=true};this.panel.querySelector('#auditRun').onclick=()=>this.runTargetAudit();this.panel.querySelector('#auditAbort').onclick=()=>{this.abort=true};this.panel.querySelector('#limitRun').onclick=()=>this.runLimitGenderAudit();this.panel.querySelector('#limitAbort').onclick=()=>{this.abort=true};this.panel.querySelector('#fit23Run').onclick=()=>this.runV323Fit();this.panel.querySelector('#fit23Abort').onclick=()=>{this.abort=true};this.panel.querySelector('#fit24Run').onclick=()=>this.runV324Fit();this.panel.querySelector('#fit24Abort').onclick=()=>{this.abort=true};this.panel.querySelector('#fit25Run').onclick=()=>this.runV325Fit();this.panel.querySelector('#fit25Abort').onclick=()=>{this.abort=true};this.panel.querySelector('#fit26Run').onclick=()=>this.runV326Fit();this.panel.querySelector('#fit26Abort').onclick=()=>{this.abort=true};this.panel.querySelector('#fit27Run').onclick=()=>this.runV327Fit();this.panel.querySelector('#fit27Abort').onclick=()=>{this.abort=true};this.panel.querySelector('#fit28Run').onclick=()=>this.runV328Fit();this.panel.querySelector('#fit28Abort').onclick=()=>{this.abort=true}}
 ...
 async repairStage328(r,tgt,key,anchors){
  const cs=this.repairControlsForMeasure(key);if(!cs.length)return {key,max:0};let max=0;
  const score=()=>{const m=this.current();let q=0;const focus=Math.abs((m[key]??0)-(tgt[key]??0));q+=focus*focus*6;for(const [k,a] of Object.entries(anchors)){if(!Number.isFinite(m[k])||!Number.isFinite(a))continue;const tol=k==='chestDepth'?.22:.30,d=Math.max(0,Math.abs(m[k]-a)-tol);q+=d*d*25}return q};
  for(const step of [.20,.10,.05,.025])for(const c of cs){let v=Number(this.engine.directState[c.id]||0),best={v,sc:score()};for(const d of [-1,1]){const nv=Math.max(-1,Math.min(1,v+d*step));this.engine.directState[c.id]=nv;await this.refreshLock(r);const sc=score();if(sc<best.sc-1e-6)best={v:nv,sc}}this.engine.directState[c.id]=best.v;await this.refreshLock(r);max=Math.max(max,Math.abs(best.v));if(Math.abs(best.v)>=.999){let last=best;for(const lim of [1.2,1.5,1.75,2]){this.engine.directState[c.id]=Math.sign(best.v)*lim;await this.refreshLock(r);const sc=score();if(sc<last.sc-1e-6){last={v:Math.sign(best.v)*lim,sc};max=Math.max(max,lim)}else{this.engine.directState[c.id]=last.v;await this.refreshLock(r);break}}}}
  return {key,max};
 }
 async fitV328Person(r){const old=await this.fitV327Person(r);if(r.gender!==0)return {old,stages:[],state:this.current()};const tgt=this.target(r),anchors={chestDepth:this.current().chestDepth},stages=[];for(const k of ['hipBreadth','waistBreadth','waistDepth','torso','shoulder','neckBase','chestBreadth']){stages.push(await this.repairStage328(r,tgt,k,anchors));const m=this.current();if(Number.isFinite(m[k])&&Number.isFinite(tgt[k])&&Math.abs(m[k]-tgt[k])<1.25)anchors[k]=m[k]}for(const k of ['hipBreadth','waistBreadth','waistDepth','torso','shoulder','neckBase','chestBreadth'])stages.push(await this.repairStage328(r,tgt,k,anchors));return {old,stages,state:this.current()}}
 async runV328Fit(){
  ...
  const snap=this.engine.snapshot(),PB=Object.fromEntries(TARGETS.map(([k])=>[k,[]])),PA=Object.fromEntries(TARGETS.map(([k])=>[k,[]])),over={},samples=[];let FB=[],FA=[],MB=[],MA=[];this.panel.querySelector('#fit28Progress').classList.remove('hidden');this.panel.querySelector('#fit28Status').textContent='LÄUFT';this.abort=false;
  try{for(let i=0;i<rows.length;i++){if(this.abort)break;const t0=performance.now(),r=rows[i];await this.baseline(r);let es=[];for(const [k] of TARGETS){const a=this.auditRaw(r,k),m=this.current()[k];if(Number.isFinite(a)&&Number.isFinite(m)){PB[k].push(Math.abs(m-a));es.push(Math.abs(m-a))}}let e=es.reduce((a,b)=>a+b,0)/es.length;(r.gender===0?FB:MB).push(e);const fit=await this.fitV328Person(r);es=[];for(const [k] of TARGETS){const a=this.auditRaw(r,k),m=this.current()[k];if(Number.isFinite(a)&&Number.isFinite(m)){PA[k].push(Math.abs(m-a));es.push(Math.abs(m-a))}}e=es.reduce((a,b)=>a+b,0)/es.length;(r.gender===0?FA:MA).push(e);for(const st of fit.stages)if(st.max>1.001)over[st.key]=(over[st.key]||0)+1;const sec=(performance.now()-t0)/1000;if(sec>0&&sec<600)samples.push(sec);if(samples.length>15)samples.shift();const done=i+1,p=100*done/rows.length,med=[...samples].sort((a,b)=>a-b)[Math.floor(samples.length/2)];this.panel.querySelector('#fit28Pct').textContent=p.toFixed(0)+'%';this.panel.querySelector('#fit28Bar').style.width=p+'%';this.panel.querySelector('#fit28Count').textContent=`${done} / ${rows.length}`;this.panel.querySelector('#fit28Eta').textContent=done>2?`ca. ${Math.round(med*(rows.length-done)/60)} min verbleibend`:'Restzeit wird geschätzt …';await new Promise(q=>setTimeout(q,0))}
  const M=a=>a.length?a.reduce((x,y)=>x+y,0)/a.length:NaN,F=x=>Number.isFinite(x)?x.toFixed(2):'—',keys=TARGETS.map(x=>x[0]),B=M(keys.flatMap(k=>PB[k])),A=M(keys.flatMap(k=>PA[k])),reg=keys.filter(k=>M(PA[k])>M(PB[k])+.20),good=A<B-.2&&M(PA.chestDepth)<=1.15&&reg.length<=1;
  this.panel.querySelector('#fit28Result').innerHTML=`<div class="optimizerHero ${good?'hit':'miss'}"><small>V3.28 · ${FA.length+MA.length} PERSONEN · MULTI-STAGE</small><strong>${good?'BEREIT FÜR FINAL-HOLDOUT':'NOCH NICHT EINFRIEREN'}</strong><span>Gesamt-MAE ${F(B)} → <b>${F(A)} cm</b> · Brusttiefe ${F(M(PB.chestDepth))} → <b>${F(M(PA.chestDepth))} cm</b></span></div><div class="batchMeasureMatrix"><b>Gesamtfehler je Geschlecht</b><span>Frauen: ${F(M(FB))} → <strong>${F(M(FA))} cm</strong></span><span>Männer: ${F(M(MB))} → <strong>${F(M(MA))} cm</strong></span></div><div class="batchMeasureMatrix"><b>Einzelmaße · vorher → V3.28</b>${TARGETS.map(([k,l])=>`<span>${l}: ${F(M(PB[k]))} → <strong>${F(M(PA[k]))} cm</strong></span>`).join('')}</div><div class="batchInfo"><b>Overdrive >100 %</b><span>${Object.entries(over).map(([k,n])=>`${k}: ${n}×`).join(' · ')||'nicht benötigt'}</span></div><div class="batchInfo"><b>Schutzprüfung</b><span>${reg.length?'Schlechter >0,20 cm: '+reg.join(', '):'Keine relevante Regression.'}</span></div><div class="batchInfo"><b>Danach</b><span>${good?'Final-Holdout ausführen; bei Bestätigung einfrieren und Kalibrierung beenden.':'Nur den ausgewiesenen Restengpass korrigieren.'}</span></div>`;this.panel.querySelector('#fit28Status').textContent=this.abort?'PAUSIERT':'ERLEDIGT'}finally{this.engine.restore(snap);this.ui.sync();this.engine.computeMetrics()}}
 async runV327Fit(){
  const rows=(this.batch?.rows||[]).filter(r=>role(r)==='validation'&&[r.height,r.weight,r.chest,r.waist,r.hip].every(Number.isFinite)).slice(0,100);
  if(rows.length<50){alert('Nicht genug Validation-Personen geladen.');return}
